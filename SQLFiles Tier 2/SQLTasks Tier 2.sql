...
FROM Members AS m
RIGHT JOIN (SELECT memid, facid FROM Bookings
GROUP BY memid, facid) AS b
USING(memid)
...
LEFT JOIN Members
USING(memid)
WHERE (CASE WHEN memid = 0 THEN slots*guestcost ELSE slots*membercost END > 30) 
        AND (starttime > '2012-09-14') 
        AND (starttime <'2012-09-15');

SELECT CONCAT(firstname,' ',surname) AS member_name FROM Members;
